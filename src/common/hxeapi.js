    ...
    return new Promise((resolve, reject) => {
        if (hxe) {
            hxe.getCode({
                ...
                // boxzhang
                // clientId: "18a17b0b-b69e-4d31-8d56-f24a9210e54a",
                // boxzhang end
                success: function(result) {
                    resolve(result.code);
                    console.log("VUE_APP_CLIENTID1：" + clientId)
                },
                fail: function() {
                    reject('获取授权码接口调用失败');
                    console.log("VUE_APP_CLIENTID2：" + clientId)
                }
            });
        } else {
            if (Vue.prototype.$config_env.VUE_APP_CODE) {
                ...
                console.log("VUE_APP_CLIENTID3：" + clientId)
            } else {

                reject('hxe对象为空，且未配置授权码');
                console.log("VUE_APP_CLIENTID4：" + clientId)
            }
        }
    })
}
