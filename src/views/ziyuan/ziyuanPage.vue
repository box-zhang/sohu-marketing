	...
	<div class="page">
		...
		<div class="anlidiv">
			...
			<div v-for="(item,index) in dataList" v-bind:key="index" @click="clickDetail(index)">
			    <div class="anliobj1" v-if="index==0&&item.status==3&&item.bigImagesUrl">
			    ...
			    <div class="anliobj" v-else>
			    	<van-image class="imagepro" fit="cover" :src="item.imagesUrl" />
			    	<div class="namediv">
			    		<div>
			    			<div class="name">{{item.name}}</div>
			    			<div class="desc" v-if="item.describes">{{item.describes}}</div>
			    		</div>
			    		<div class="timer">
			    			<van-image class="timerimage" fit="cover" :src="require('../../assets/static/home/icon_shijian.png')" />
			    			<div class="text">{{item.dateTime}}</div>
  ...
  data() {
   ...
   methods: {
	   ...
	   getIndexZiyuan : function(){
	   	...
	   	this.ifloading = true
	   	var params = {
	   		ziyuanTypeId:this.dataobj.id,
	   		pageSize:88,
	   		currentPage:this.currentPage
	   	}
	   	console.log(params)
	   	this.$http.post('zhiyuanModule/index-ziyuan/getIndexZiyuan',params)
