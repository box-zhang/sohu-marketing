...
export default {
  ...
  created() {
	  ...
	  if(dataobj){
			 ...
			 //直接打开文档
			 if(this.dataobj.iffile=="yes"){
				var responseobj = this.dataobj
				this.title = responseobj.name
				//过滤所有空格
				...
				if(!responseobj.outUrl&&responseobj.fileUrl){
					...
					if(userinfo){
						...
						console.log("水印")
						this.$watermark.set(userinfo.emplId,userinfo.emplName)
					}
  ...
  beforeDestroy(){
   ...
   methods: {
	  ...
	  // 获取菜文章
	  getziyuanArticlebyid : function(){
	  		var _this = this
	  		var params = {
	  			id:this.dataobj.uid
	  		}
	  	this.$http.post('zhiyuanModule/index-ziyuan/getById',params)
