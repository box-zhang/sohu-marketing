	...
	<div class="page">
		...
		<van-swipe class="my-swipe" :autoplay="3000" indicator-color="white">
		    <van-swipe-item v-for="(item, index) in bannerList" :key="index">
		      <van-image fit="cover" :src="item.imageUrl" class="swipeimg" @click="clickbanner(index)"></van-image>
		    </van-swipe-item>
		</van-swipe>
		<div class="block20"></div>
		<!-- 菜单栏目 -->
		<van-swipe style="padding-bottom: 18px;" indicator-color="#333333" @change="swipeTab" :loop="false">
		    <van-swipe-item v-for="(fitem, findex) in barlist" :key="findex" class="tabswipe">
		...
		<div class="ziyuandiv" v-if="ziyuanbar.length>0">
			...
			<div class="zytop">
				<div class="left" :style="'background-image: url('+ziyuanbar[0].imagesUrl+');'" @click="clickziyuan(0)">
					<div class="title1">{{ziyuanbar[0].name}}</div>
					<div class="title2">{{ziyuanbar[0].describes}}</div>
				</div>
				<div class="right" v-if="ziyuanbar.length>1">
					<div class="right1" v-if="ziyuanbar.length>1" :style="'background-image: url('+ziyuanbar[1].imagesUrl+');'" @click="clickziyuan(1)">
						<div class="title1">{{ziyuanbar[1].name}}</div>
						<div class="title2">{{ziyuanbar[1].describes}}</div>
					</div>
					<div class="right2" v-if="ziyuanbar.length>2" :style="'background-image: url('+ziyuanbar[2].imagesUrl+');'" @click="clickziyuan(2)">
			...
			<div class="zybottom">
				<div class="bottom1" v-for="(item,index) in ziyuanbar" :key="index" v-if="index>2" :style="'background-image: url('+item.imagesUrl+');'" @click="clickziyuan(index)">
		...
		<div class="newsdiv">
			...
			<div v-for="(item,index) in newsList" v-bind:key="index" @click="clickDetail(index)">
   ...
   methods: {
	   ...
	   getCode(){
	     ...
	     this.$hxe.getCode().then(data=>{	
			...
			this.$http.post('userMassage/getSouhuOpenId',params)
			  .then(function (response) {
				  ...
				  if(response.code==200){
					 ...
					 if(response.data.dataObject){
					    ...
					    localStorage.setItem("userinfo",JSON.stringify(response.data.dataObject))				 
					 }else{
	   ...
	   GetWheelByType : function(){
		...
		var params = {
	   	...
	   	this.$http.post('xcx-wheel/GetWheelByType',params)
	      .then(function (response) {
	          //数据处理
			  if(response.code==200){
	   ...
	   getIndexType : function(){
	   	...
	   	this.$http.post('indexModule/index-type/getIndexType',params)
	      .then(function (response) {
	          //数据处理
	   			  if(response.code==200){
	   ...
	   getIndexZiyuanType : function(){
	   	...
	   	this.$http.post('zhiyuanModule/index-ziyuan-type/getIndexZiyuanType',params)
	      .then(function (response) {
	          //数据处理
	   			  if(response.code==200){
	  ...
	  clickziyuan(index){
		...
		if(obj.outType==0){
			...
			dataobj["iffile"] = "no"
			this.$router.push({path: "/ziyuanPage",query:{ dataobj:encodeURIComponent(JSON.stringify(dataobj))}});
		}
	  },
	  //点击菜单
	  clickbaritem(index){
		 ...
		 if(obj.outType==0){
			 ...
			 if(obj.name.indexOf("新闻")>-1){
				this.$router.push({path: "/newsPage",query:{ dataobj:encodeURIComponent(JSON.stringify(dataobj))}});
			 }else{
				this.$router.push({path: "/proPage",query:{ dataobj:encodeURIComponent(JSON.stringify(dataobj))}});
			 }
	  		 ...
	  		 if(obj.redirectUrl){
	  			 if(obj.outType==1){
	  			 	if(obj.redirectUrl=="newsDetail"){
						...
						console.log(params)
						this.$http.post('/zhiyuanModule/index-ziyuan-type/getById',params)
						  .then(function (response) {
						      //数据处理
								  if(response.code==200){
										const dataobj = {
										...
										if(response.data.outType==0){
											dataobj["iffile"] = "yes"
											_this.$router.push({path: "/ziyuanDetail",query:{ dataobj:encodeURIComponent(JSON.stringify(dataobj))}});
										}else{
											dataobj["iffile"] = "no"
											_this.$router.push({path: "/ziyuanPage",query:{ dataobj:encodeURIComponent(JSON.stringify(dataobj))}});
										}
								  }else{
									  _this.$hxe.showModal({
									    title:"请求失败",
