<template>
	<div>
		<div class="t-calendar__box">
			<div class="t-calendar__wrapper">
				<div class="t-calenda__content">
					<div class="t-calendar__panel">
						<div class="t-calendar__date-befor" @click="dataBefor('0', 'month')">
							<van-icon name="arrow-left" />
						</div>
						<div class="t-calendar__panel-box">
							<div>{{ canlender.year }}年</div>
							<div>{{ canlender.month }}月</div>
						</div>
						<div class="t-calendar__date-after t-calendar__rollback" @click="dataBefor('1', 'month')">
							<van-icon name="arrow-left" />
						</div>
						<div class="t-calendar__backtoday" @click="backtoday">回到今天</div>
					</div>
					<div v-if="lunar" class="t-calendar__day-detail">
						<div>
							{{
								canlender.year +
									'年' +
					...
					<div class="t-calendar__header">
						<div class="t-calendar__week">日</div>
						<div class="t-calendar__week">一</div>
						<div class="t-calendar__week">二</div>
						<div class="t-calendar__week">三</div>
						<div class="t-calendar__week">四</div>
						<div class="t-calendar__week">五</div>
						<div class="t-calendar__week">六</div>
					</div>
					<calendar-item v-if="slide === 'none'" :canlender="canlender" :lunar="lunar" @selectDays="selectDays"></calendar-item>
					<van-swipe v-else class="t-calendar__body" :style="{ height: domHeihgt + 'px' }" :initial-swipe="currentIndex" 
					 :vertical="slide === 'vertical' ? true : false" :show-indicators="false" @change="animationfinish">
						<van-swipe-item v-for="(item, itemindx) in swiperData" :key="itemindx">
							<div :class="elClass" class="t-calendar__area">
								<calendar-item :canlender="item" :lunar="lunar" @selectDays="selectDays"></calendar-item>
							</div>
						</van-swipe-item>
					</van-swipe>
				</div>
...
<script>
	...
	export default {
		...
		data() {
			const elClass = `t_${Math.ceil(Math.random() * 10e5).toString(36)}`;
			return {
		...
		methods: {
			...
			getQueryDom(index) {
				let weeks=this.swiperData[index].weeks.length;
				this.domHeihgt=41*weeks;
			}
		}
	};
</script>
	...
	.t-calendar__wrapper {
		...
		.t-calenda__content {
			.t-calendar__panel {
				...
				.t-calendar__date-befor,
				.t-calendar__date-after {
					display: flex;
					justify-content: center;
					align-items: center;
					height: 40px;
					...
					&.t-calendar__rollback {
						transform: rotate(180deg);
					}
				}

				.t-calendar__panel-box {
					display: flex;
					justify-content: center;
					align-items: center;
					width: 100px;
			...
			.t-calendar__area{
				border-bottom:1px solid @neutral-split-color;
				padding-bottom:4px;
			}
			.t-calendar__day-detail {
				padding: 10px;
				padding-left: 15px;
				border-top: 1px @neutral-split-color solid;
			}
