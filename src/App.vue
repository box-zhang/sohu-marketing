...
export default {
	...
	created() {
		...
		if (this.$config.adjustContentBottomInset) {
			this.$hxe.adjustContentBottomInset(this.$config.adjustContentBottomInset).catch(error => {
				console.log(error);
			});
		}
	},
	...
	methods: {
		...
		onSelect(item, index) {
			if (window.onSelectActionSheet && typeof window.onSelectActionSheet == 'function') window.onSelectActionSheet(item, index);
		}
	},
	components: {
